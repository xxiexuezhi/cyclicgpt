...
@dataclass(init=True, repr=True, eq=False, frozen=False)
class LMDataModule2(LightningDataModule):
...
dataclass(init=True, eq=False, repr=True, frozen=False)
class LMDataset3(Dataset):
    """Dataset for SMILES strings, accepts list directly (not just filename)."""

    smiles_strings: Union[str, List[str]]
    tokenizer: BaseTokenizer

    def __post_init__(self) -> None:
        if isinstance(self.smiles_strings, str):  # it's a file path
            self.smiles_strings = Path(self.smiles_strings).read_text(encoding='ascii').splitlines()
